...
New structural models can export `structureControls` using the same descriptor fields with meaningful names, such as `pegReach` and `pegRise`. The viewer discovers those keys instead of requiring every control to be an arch. Register pedestal and menu-frame dimensions use this path too. Controls are validated before display; configuration remains owned by each model.

Café browser previews use the contrast light ratios without the unrelated generic studio lights. Browser exposure is calibrated separately (3.2) from Dawn captures (1.15); matching settings alone did not produce matching brightness across those output paths. This changes preview lighting only, not installed materials. The coat-stand sliders, hex tint and resets were exercised in the live browser after integration.
