...
import { catalog, findModel, f1Catalog, kyotoKatCatalog, scifiCatalog } from './catalog.ts'
...
function ModelIndex() {
  ...
  const pool = kit === 'f1' ? f1Catalog : kit === 'scifi' ? scifiCatalog : kit === 'kyoto-kat' ? kyotoKatCatalog : catalog
  const filtered = pool.filter((model) => `${model.name} ${model.category}`.toLowerCase().includes(needle))
  const eyebrow = kit === 'f1' ? 'F1 Kit' : kit === 'scifi' ? 'Sci-Fi Kit' : kit === 'kyoto-kat' ? 'Kyoto Kat Kit' : 'Source kits · MIT licensed'
  const title = kit === 'f1'
    ? 'F1 Kit models.'
    : kit === 'scifi'
      ? 'Sci-Fi Kit models.'
      : kit === 'kyoto-kat'
        ? 'Kyoto Kat Kit models.'
...
function KyotoKatKitPage() {
  const count = kyotoKatCatalog.length
  return <div className="kit-page"><PageIntro eyebrow="Reference library · MIT licensed" title="Kyoto Kat Kit"><p>{count} procedural 1990s Kyoto cat-café furnishings and props, built for Three.js and shipped as source.</p></PageIntro><CodeBlock>{'bunx vibe3d add @kyoto-kat'}</CodeBlock><div className="kit-stats"><div><b>{count}</b><span>models</span></div><div><b>Three.js</b><span>engine</span></div><div><b>MIT</b><span>license</span></div></div><Link className="primary-button" to="/models?kit=kyoto-kat">Browse Kyoto Kat models <ArrowRight /></Link></div>
}
  ...
  return (
    <Routes>
      ...
      <Route element={<DocumentationShell />}>
        ...
        <Route path="/kits/kyoto-kat" element={<KyotoKatKitPage />} />
      </Route>
